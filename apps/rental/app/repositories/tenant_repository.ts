 ...
 * Bound in app/providers/app_provider.ts. `findByDomain` is what the
 * `domain-or-subdomain` resolver calls to turn `acme.localhost` into a tenant.
 */
export default class TenantRepository implements TenantRepositoryContract {
  ...
  /**
   * Counts grouped by status, computed in the database. The package's `/metrics`
   * collector prefers this over `all()` so a Prometheus scrape stays O(1)
  ...
  async each(
    ...
    // Keyset cursor on the primary key, not OFFSET pagination: a callback that
    // mutates the rows being iterated would shift an offset window and silently
