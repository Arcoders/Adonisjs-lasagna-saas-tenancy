...
| [svc#16] | Method: `registerWebhook(tenantId, url, events, secret?) → Promise<{ hook, generatedSecret? }>` — Validates URL against SSRF guard | B | VERIFIED | webhook_service.ts registerWebhook() (SSRF guard via url.ts) | webhook_service.spec.ts, webhook_secret_generation.spec.ts | new-test:T1 (encoding matrix) | return shape changed post-audit (typed one-time secret, was $extras) |
