...
/**
 * Provider for `@adonisjs-lasagna/sso`, built with the {@link definePlugin}
 * facade. SSO is a LIBRARY satellite: the host wires the auth/callback routes and
 * resolves `SsoService` itself, so this provider declares no seams and binds no
 * singletons. Its sole job is the runtime backstop: the facade asserts the
 * Satellite ABI and the plugin-API contract at `boot()`, so a later core downgrade
 * underneath an already-configured host fails fast instead of drifting silently.
 *
...
export default definePlugin({
  name: 'sso',
  packageName: '@adonisjs-lasagna/sso',
