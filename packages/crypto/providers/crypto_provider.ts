...
import { WormLedgerWriter, type WormDb } from '@adonisjs-lasagna/saas-tenancy/internal'
 ...
 *
 * The facade wires the ABI backstops (the Satellite ABI and the plugin-API contract)
...
export default definePlugin({
  ...
  bind(app) {
    ...
    // default. A host binds its own aws-kms, hashicorp-vault, or custom provider by
    // resolving this registry and calling `register(...)` in its own provider.
    // Stateful (Map-backed): a container singleton, never new-ed ad hoc.
    ...
    app.container.singleton(CryptoService, async (resolver) => {
      ...
      // erasability gate is wired from governance's config seam; when it is absent a
      // shred is fail-closed refused. encrypt and decrypt do not depend on either.
      // Both the schema and the connection are resolved from config (never a
      // hardcoded literal): the ledger's SQL is qualified through
      // qualifyBackofficeTable with `backofficeSchemaName`, and it runs on
      // `backofficeConnectionName`, the convention core uses for every
      // backoffice-schema table, so a host that separates or renames its backoffice
      // schema or connection is honored.
    ...
    // The KEK-rotation walker driving `tenant:crypto:rekek`. It shares the same
    // KeyProvider and Pg store wiring as CryptoService (a fresh store: the store is
    // stateless behind its injected deps), and re-wraps DEKs under the current KEK
    // generation without ever decrypting a field value. Resolved via
    ...
    // The explicit field-encryption surface: a context-aware facade over
    // CryptoService that resolves the current tenant per call, so the caller passes
    // only `(subject, category)` and the value. Resolved via
