...
/**
 * The mixin that makes `@encrypted` / `@searchable` columns transparent. Compose it
 * onto whichever base model the host uses:
 *
 * ```ts
 ...
 * ```
 *
 * It registers async Lucid lifecycle hooks (the DEK unwrap is async, which the
 * synchronous `prepare`/`consume` column hooks cannot do): `before('create'|'update')`
 * encrypts and (re)indexes, failing closed (a failure aborts the save), and
 * `after('create'|'update'|'find'|'fetch')` decrypts back to plaintext in memory
 * (failing closed on a shredded or tampered value). The engine is the container
 ...
 *
 * The scope of the guarantee, and its honest limits. These hooks fire only on the
 * model *instance* write/read path: `model.save()`, `Model.create()` / `createMany()`,
 * and loads via `find`/`fetch`/`paginate`. They do not fire for:
 ...
 *     and skip the blind index (a plaintext bypass; the DB-level fail-closed guard the
 ...
 * fail-closed and aborts the whole batch (there is no per-row isolation, by design).
 * The host must null/soft-delete the encrypted column (or filter the row out) on
 ...
 * host `TenantRepository.findById` with a cache. See the design notes on honest bounds.
 */
