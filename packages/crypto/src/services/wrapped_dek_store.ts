...
/** A persisted wrapped-DEK row. Holds the DEK only in wrapped form, never as plaintext. */
 ...
 * excluded by the partial UNIQUE.
 */
export interface WrappedDekStore {
   ...
   * Shredded tombstones are excluded (they hold no key to re-wrap). The
   ...
   * the live DEK singular, so a racing second provision throws rather
   ...
   * KEK-rotation walker after it unwraps the DEK under the old KEK and
