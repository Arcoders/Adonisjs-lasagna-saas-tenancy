 ...
 * where the tenant's wrapped DEKs physically live and runs parameterized
...
export default class PgWrappedDekStore implements WrappedDekStore {
  ...
  async rewrap(
    ...
    // material. The DEK value is unchanged; only its KEK wrapping rotates.
    // safe-sql: `table` is a fixed module constant and `scope.column` is a driver-
  ...
  async insert(tenant: TenantModelContract, row: NewWrappedDekRow): Promise<WrappedDekRow> {
    ...
    try {
      ...
      // silent `undefined` deref would violate the fail-closed contract.
      ...
      // a racing second provision is a 23505, surfaced fail-closed so the
  ...
  async shredLive(
    ...
    // the only copy of the key. Only the live row is touched (WHERE
  ...
  async #target(tenant: TenantModelContract): Promise<StoreTarget> {
    ...
    if (active !== undefined && active !== tenant.id) {
      ...
      // request tenant equals the active scope before any query (the cross-tenant guard).
