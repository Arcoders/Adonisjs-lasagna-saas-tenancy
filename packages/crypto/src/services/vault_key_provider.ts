...
export interface VaultKeyProviderOptions extends HttpKeyProviderOptions {
  /** The Vault base address, e.g. `https://vault.internal:8200`. Routed through safeFetch (T13). */
   ...
   * tenant's key material is rotated/destroyed independently (foundation §2.1 "ideally
   * per-tenant"). Pre-create the keys, or enable Vault's upsert. Default `lasagna-crypto-`.
   */
 ...
 * `transit/decrypt/<key>`, so raw KEK bytes never enter the app process (I2's T2
 * mitigation, unlike the dev `env` provider). It extends {@link HttpKeyProvider}, so
 * EVERY outbound call is pinned by core `safeFetch` (T13) — a mis-set `address` cannot
