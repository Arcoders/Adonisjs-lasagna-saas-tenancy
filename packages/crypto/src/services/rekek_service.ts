...
/**
 * The KEK-rotation walker. Rotating the KEK does NOT re-encrypt field
 ...
 * `secrets_rotation.ts` but not its function: that classifies
...
export default class RekekService {
  ...
  async rekekTenant(
    ...
    // unwrap. When it is absent (a provider that cannot report it), the walker
    // falls back to post-hoc classification.
    ...
    for (;;) {
      ...
      for (const row of rows) {
        ...
        // Re-wrap: unwrap the DEK under whatever KEK generation the provider
        ...
        // the same generation, so count it as current and write nothing.
