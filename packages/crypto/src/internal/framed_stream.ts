...
/**
 * The framed enc_v2 stream envelope: a composition of core's `sealV2WithKey` and
 * `openV2WithKey` per frame, not a new cipher. Each frame's non-secret enc_v2
 ...
 * {@link FRAMED_STREAM_PREFIX} followed by `\n`-joined frames). O(payload) but
 * whole-buffer; for multi-GB blobs use {@link sealFramedV2Stream}.
 */
 ...
 * DEK. It never returns partial plaintext, so the read fails closed.
 */
...
/**
 * Streaming seal. The motivating case: `vault` encrypts a multi-GB blob before
 * upload. Re-chunks the source into fixed-size frames and yields each sealed frame
 * string, then a terminator. The consumer persists frames in order (e.g. one object
 * part each); the container prefix and newline joining are the single-shot form's
