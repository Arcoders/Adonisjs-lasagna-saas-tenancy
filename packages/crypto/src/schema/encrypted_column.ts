/**
 * The DB-level fail-closed backstop that rejects a plaintext write to an
 * `@encrypted` column: a Postgres CHECK constraint that refuses any non-ciphertext
 * value written to it, at the database layer. This is the only enforcement that
 ...
 *
 * A Lucid `prepare` column hook would catch the `*Quietly` and direct-attribute
 * cases but not raw SQL; only a database constraint covers every path. So this closes
 * the gap the decorator alone cannot: a field marked encrypted written as cleartext.
 *
 ...
 * provider-independent prefix and is deliberately not constrained here (the
 * wrapped-DEK column is covered separately by `check-crypto-invariant-2`).
 */
 ...
 * the fail-closed backstop for every write path, including the raw-SQL and
