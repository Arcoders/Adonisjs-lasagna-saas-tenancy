/**
 * The per-tenant operation-lock seam. Provision and shred serialize on it so two
 * concurrent writes to one `(subject × category)` DEK cannot interleave. It is
 * injected into {@link ../services/crypto_service.js CryptoService} rather than
 ...
 *
 * When it is absent the critical sections run without cross-process serialization.
 * That is a safe degraded mode, not a silent hole: the partial `UNIQUE (subject_id,
 * category) WHERE shredded_at IS NULL` is the real singularity guarantee (a racing
 * second provision is refused fail-closed at the DB), and the lock is
 * defense-in-depth that turns a hard conflict into clean serialization. Unit tests
 * inject a fake to assert the critical sections are wrapped.
 */
...
/** Per-call lock options. Absent means `{ onContention: 'fail-open' }`. */
