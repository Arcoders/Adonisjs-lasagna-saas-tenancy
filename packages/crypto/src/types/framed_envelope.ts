/**
 * The framed enc_v2 stream envelope (crypto ARCHITECTURE.md §6.8). A large blob cannot
 * be sealed as a single GCM tag (the tag is unverifiable until the last byte, so a
 * multi-GB payload cannot stream), and inventing a second chunked AEAD is exactly what
 * the foundation forbids. So crypto owns a COMPOSITION of core's existing enc_v2 GCM
 * primitive: the payload is split into fixed-size frames, each sealed as an independent
 ...
 * reordered / dropped / duplicated / cross-stream frame fails authentication, plus a
 * final terminator frame carrying the authenticated frame count so a truncated stream
 * is detected rather than silently accepted. There is ONE AEAD and it is core's
 * (foundation §2.2); this is that primitive applied per frame with reorder/truncation
 * binding, NOT a new cipher (I1 holds).
 *
 * `vault` composes this to encrypt blobs before upload; it does not scope its own
 * envelope (ARCHITECTURE §6.8, §12 decision 5).
 */
