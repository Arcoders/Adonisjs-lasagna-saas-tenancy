...
/**
 * Crypto-shred a `(subject × category)`: the O(1) erasure that destroys the wrapped
 * DEK, making every field ciphertext and vault blob under it (and their backups)
 * irrecoverable at once. Operator-privileged and gated:
 *
 ...
 *
 * The erasure is refused when governance's `legalBasis` marks the category
...
export default class TenantCryptoShred extends BaseCommand {
  ...
  async run() {
    ...
    try {
      ...
      // A refusal (legal hold, governance absent, or unaudited) is the fail-closed
      // path: report it clearly and exit non-zero rather than throwing a stack.
