...
/**
 * KEK rotation: re-wrap every live DEK under the current KEK generation. Rotating
 * the KEK does NOT re-encrypt field values. It unwraps each DEK under the old KEK
 * and re-wraps it under the new one, so the operation is O(number of DEKs), not
 * O(number of field values), and the data ciphertext keeps decrypting.
 *
 ...
 * A KMS or Vault provider retains its prior key versions itself, so no env is needed.
 *
...
export default class TenantCryptoRekek extends BaseCommand {
  ...
  /** Rotate one tenant inside its tenancy scope so the store's ContextSeal and connection resolve. */
  ...
  /** Print the per-tenant and total summary; any failed DEK is a non-zero exit. */
