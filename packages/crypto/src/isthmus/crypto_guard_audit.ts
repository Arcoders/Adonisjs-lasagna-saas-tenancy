 ...
 *
 * The factory gives each instance its OWN windows and counters (same divergence
 * as AI): a crypto-surface burst cannot consume the kernel's per-severity
 * dispatch budget. The budget VALUES are the shared `ISTHMUS_BUDGETS` inside the
 * factory, so both layers stay tuned together on a kernel retune.
 *
 ...
 * metadata (I9): call sites pass only non-secret ids / sizes.
 */
