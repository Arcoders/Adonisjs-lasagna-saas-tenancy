 ...
 *
 * - `guard.crypto_plaintext_write`: a plaintext write to an `@encrypted` column is
 *   refused at the database layer by the `enc_v2:`/`enc_v1:` prefix CHECK
 *   (src/schema/encrypted_column.ts), the only enforcement that catches the raw-SQL
 *   or query-builder bypass. crypto never sees that write to emit on it, so there is
 *   no app-level entry: the DB constraint is the guard.
 * - `dek_missing` / `no_tenant_scope` / `dek_conflict`: real fail-closed throws, but
 *   availability or API-shape errors, not refusals of a security decision with a
...
interface CryptoGuardRegistryEntryShape {
  ...
  /** 'config' trips at boot or validation and aborts the deploy; 'runtime' is per request. */
