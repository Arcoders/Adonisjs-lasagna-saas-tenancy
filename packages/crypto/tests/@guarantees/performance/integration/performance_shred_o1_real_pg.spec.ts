...
/**
 * PERFORMANCE guarantee — the crypto-shred is O(1) and per-write cost does not grow
 * the key store (crypto §6.3/§6.6, I6). These are the load-bearing SCALE properties
 * of crypto-shredding, asserted deterministically by ROW/OPERATION count (never
 * wall-clock, which flakes): the cost of erasure is one key delete regardless of how
 * much data it sealed, and writing more values under a subject does not multiply keys.
 *
 * 1. Per-write cost is O(1) in the key store: encrypting one `(subject × category)`
 *    N times reuses the SAME live DEK — exactly one wrapped-DEK row exists, not N.
 ...
 *
 * Runs against the real PgWrappedDekStore + WORM ledger; self-skips without Postgres,
...
test.group('crypto shred is O(1) — real PgWrappedDekStore + WORM ledger', (group) => {
  group.setup(async () => {
    ready = await probePg()
    if (!ready) return
