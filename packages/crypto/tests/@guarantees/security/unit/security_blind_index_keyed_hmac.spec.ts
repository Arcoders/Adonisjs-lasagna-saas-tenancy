...
// I5 (§6.5): equality search uses a KEYED HMAC (a blind index), not a bare salted
// hash. Equal plaintexts index equally (so equality search works), the key lives
// in the KeyProvider (so a DB dump cannot brute-force it, T3), the index key is
// distinct from the DEK and SURVIVES a crypto-shred (T14), and the documented
// equality/frequency leak is an honest invariant, never silent.
test.group('crypto blind index — keyed HMAC for equality search (I5)', (group) => {
  group.each.setup(() => {
  ...
  test('it is KEYED, not a bare hash: it does not match sha256(value) (T3)', async ({ assert }) => {
  ...
  test('a different APP_KEY yields a different index key, so the index changes (T3)', async ({
    assert,
  }) => {
    const value = 'passport-AB1234567'
  ...
  test('the blind index SURVIVES a crypto-shred: the index key is not the DEK (T14)', async ({
    assert,
  }) => {
