...
/**
 * The DB-level ciphertext CHECK (crypto §4 I3, §5 invariant-3) on REAL Postgres: the
 * constraint `encryptedColumnCheckSql` emits is the fail-closed backstop that closes
 * T5 ("write a field marked encrypted as cleartext") for the write paths the model
...
test.group('crypto encrypted-column CHECK on real Postgres (T5 backstop)', (group) => {
  group.setup(async () => {
    ready = await probePg()
    if (!ready) return
    T = randomUUID()
