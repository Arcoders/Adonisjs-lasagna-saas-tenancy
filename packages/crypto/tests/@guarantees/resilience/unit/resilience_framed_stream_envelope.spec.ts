...
// The framed enc_v2 stream envelope composes core's GCM primitive per frame (it is
// not a new cipher), with a monotonic frame counter bound into the authenticated keyId
// and a terminator carrying the frame count. Every integrity fault (reorder, drop,
// duplicate, truncation, tamper, wrong DEK, cross-stream splice) must fail closed, and a
// healthy payload round-trips byte-for-byte.
test.group('crypto framed enc_v2 stream envelope: integrity is fail-closed', () => {
