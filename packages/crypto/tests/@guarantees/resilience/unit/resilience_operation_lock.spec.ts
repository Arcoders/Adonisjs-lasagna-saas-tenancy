...
test.group('resilience: per-tenant operation lock', (group) => {
  group.each.setup(() => {
  ...
  test('provisioning a DEK runs under the lock; reusing a live DEK does not', async ({
    ...
    await service.encryptField(t, 'renter-1', CAT, 'passport-1') // provisions, so one lock
  ...
  test('shred runs under the lock', async ({ assert }) => {
    ...
    await service.encryptField(t, 'renter-1', CAT, 'passport-1') // provision takes the lock (1)
    const result = await service.shred(t, 'renter-1', CAT) // shred takes it again (2)
  ...
  test('a dry-run shred does NOT take the lock (it destroys nothing)', async ({ assert }) => {
    ...
    assert.isFalse(dry.alreadyShredded) // a live DEK exists, so it would shred
  ...
  test('concurrent first-writes to one (subject × category) resolve to ONE DEK, no conflict', async ({
    ...
    // Exactly one live DEK row exists for the pair.
  ...
  test('an absent lock is a safe degraded mode: encrypt/shred still work', async ({ assert }) => {
    // No withLock wired, so the partial UNIQUE and the idempotent shred are the backstop.
    const { service } = makeService({
      erasabilityResolver: erasable(),
      ledger: new RecordingLedger(),
    })
