...
// KeyProvider down (KMS or Vault unreachable) must fail closed. A DEK that
// cannot be unwrapped makes the read fail; it never falls back to a shared or plaintext
// key. A DEK that cannot be wrapped makes the write fail; a new encrypted value is never
