...
/**
 * Wrap a real in-memory store so `shredLive` reports that it tombstoned nothing. That is
 * the exact signal that a concurrent shred won the race and already destroyed the DEK
 * between our `findLive` and our `shredLive` (only reachable in the Redis-down degraded
 * lock mode).
 */
...
// Under contention the shred serializes on the operation lock, but if
...
// marker and emits SubjectShredded. A loser (shredLive tombstones no rows) returns
// alreadyShredded and never double-audits or double-emits; its PENDING row is a detectable orphan.
test.group('crypto shred: concurrent-race authority', (group) => {
  group.each.setup(() => {
