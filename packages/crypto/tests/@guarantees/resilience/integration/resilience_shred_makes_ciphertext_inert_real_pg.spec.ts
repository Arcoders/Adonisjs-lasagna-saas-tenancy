...
/**
 * The I6 enforcement on REAL Postgres (the design's `resilience_shred_makes_
 ...
 * lands a PENDING then a COMMITTED row in the real `backoffice.worm_ledger`. Also
 * proves the I7 legal-hold refusal (the DEK survives) and the I10 re-provision after
...
test.group('crypto shred makes ciphertext inert (real pg + WORM ledger)', (group) => {
  ...
  test('a legal-obligation category in retention is refused; the DEK survives (I7)', async ({
    assert,
  }) => {
  ...
  test('a re-provision after a shred inserts a fresh live DEK (I10 partial unique)', async ({
    assert,
  }) => {
    const svc = serviceAs(T, { routes, withLedger: true, erasabilityResolver: erasable() })
