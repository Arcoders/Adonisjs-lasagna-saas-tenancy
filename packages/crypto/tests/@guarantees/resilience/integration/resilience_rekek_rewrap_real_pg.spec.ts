...
/**
 * KEK rotation (I8, §6.7) on REAL Postgres through the real PgWrappedDekStore +
 ...
 * walker. It asserts the load-bearing I8 properties end-to-end:
 ...
 *     never silently rotated (fail-closed).
...
test.group('crypto KEK rotation (rekek) on real Postgres', (group) => {
  ...
  test('re-wraps every DEK under the new KEK; data still decrypts; idempotent', async ({
    ...
    // 2. Rotate: previous key → OLD_APP_KEY, a new key → APP_KEY.
    ...
    // bytes were preserved — the ciphertext was never re-encrypted).
  ...
  test('a DEK the provider can no longer unwrap is reported failed, then recovers via OLD_APP_KEY', async ({
    ...
    // Provision under the current key; remember it — it is the key that wrapped
