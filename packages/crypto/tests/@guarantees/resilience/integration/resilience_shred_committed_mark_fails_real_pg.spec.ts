...
/**
 * §6.6 / §9, the crash-between-PENDING-and-COMMITTED failure mode, on REAL Postgres with
 ...
 * two (here: the COMMITTED append throws), the DEK is already destroyed (correct — the
 * erasure is irreversible), and a DETECTABLE PENDING row remains in the ledger with no
 * matching COMMITTED marker, so a reconciliation pass / operator can find it. The erasure
 * is auditable; it is just not yet finalized. Never a silent success. Self-skips when
 * Postgres is unreachable (local), runs in CI.
 */
...
test.group('crypto shred — crash between PENDING and COMMITTED (real Postgres)', (group) => {
  ...
  test('the DEK is destroyed and an un-committed PENDING row remains for reconciliation', async ({
    ...
    try {
      ...
      // The shred completes the delete but the COMMITTED mark crashes → reported, not silent.
