 ...
 * least-privilege role (crypto §6, I1/I4 defence-in-depth). The sibling smoke spec
 ...
 *
 * Self-skips locally (central + rls_probe both resolve to the superuser → RLS not
 * enforced), and — mirroring core's RLS proof — fails LOUD rather than skipping when
...
test.group('crypto wrapped-DEK rowscope RLS ENFORCED under least-privilege (real pg)', (group) => {
  group.setup(async () => {
    ...
    // Create the shared rowscope table WITH the shipped ENABLE/FORCE RLS + policy,
  ...
  test('the shipped store round-trips a field under the SAME enforcing role (set_config path)', async ({
    ...
    // (USING). A store bug that forgot set_config would fail closed here — this is
