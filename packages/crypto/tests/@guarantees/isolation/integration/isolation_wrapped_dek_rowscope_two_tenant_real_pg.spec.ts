...
/**
 * The isolation proof for the `rowscope-pg` placement on real Postgres: one shared
 * `crypto_wrapped_deks` table on the central connection, tenant separation by the
 * `tenant_id` scope column the store stamps and filters. Unlike schema-pg/database-pg,
...
test.group('crypto wrapped-DEK rowscope two-tenant isolation (real pg)', (group) => {
  ...
  test('the same (subject × category) coexists per tenant and derives independent DEKs', async ({
    assert,
  }) => {
    const svcA = rowscopeServiceAs(A)
    const svcB = rowscopeServiceAs(B)

    // Same subject id, category, and plaintext in both tenants: no dek_conflict, because
  ...
  test('the per-(tenant, subject, category) partial UNIQUE allows re-provision after a shred', async ({
    ...
    // A now has a tombstone and a fresh live row; its live lookup returns the new one.
    const live = await storeA.findLive(tenant(A), 'renter-9', CAT)
    assert.equal(live?.wrappedDek, 'enc_v2:a2')
    assert.equal(await countRows('WHERE tenant_id = ?', [A]), 2) // tombstone and live
 ...
 * (a transaction plus a set_config of the GUC) works end-to-end against a real
 * RLS-enabled table. Enrolling the stub's ENABLE/FORCE ROW LEVEL SECURITY and policy
 * verbatim, then driving the store with rls:true, proves: the policy DDL parses and
 * applies, the GUC name agrees with the store's rlsGuc, and set_config(is_local=true)
 * lets the store's own INSERT/SELECT pass the policy's USING/WITH CHECK. It does NOT
 * assert cross-tenant blocking, because FORCE RLS is bypassed for a superuser (the
 * likely local role), and RLS enforcement is core's tested concern
 * (isolation_rowscope_rls.spec). Runs wherever PG is reachable.
 */
let rlsReady = false
...
test.group('crypto wrapped-DEK rowscope RLS smoke (real pg)', (group) => {
  ...
  test('the store round-trips a field under the stub’s FORCED RLS policy (set_config path)', async ({
    ...
    // would make this fail closed (zero rows, so 'no live DEK').
