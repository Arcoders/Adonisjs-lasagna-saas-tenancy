...
/**
 * The isolation proof on real Postgres: two tenants placed in two schemas via
...
test.group('crypto wrapped-DEK two-tenant isolation (real pg)', (group) => {
  ...
  test('the same (subject × category) in both tenants derives independent DEKs', async ({
    assert,
  }) => {
    const svcA = serviceAs(A, { routes })
    const svcB = serviceAs(B, { routes })

    // Same subject id, category, and plaintext in both tenants.
