...
/**
 * The blind index enabling equality search on real Postgres: a host stores the keyed
 * HMAC in its own index column and queries `WHERE idx = ?`. This proves equality
 * search survives encryption end-to-end, that equal plaintexts share an index (the
 * documented frequency leak), and that the index survives a crypto-shred. Once the
 * DEK is destroyed the value is undecryptable, yet the stale index still matches until
 * the host nulls the column. Self-skips when Postgres is unavailable, runs in CI.
 */
...
test.group('crypto blind-index equality query (real pg)', (group) => {
  ...
  test('the blind index survives a shred: undecryptable but still equality-matchable', async ({
    assert,
  }) => {
    ...
    // equality stays computable until the host nulls the column.
