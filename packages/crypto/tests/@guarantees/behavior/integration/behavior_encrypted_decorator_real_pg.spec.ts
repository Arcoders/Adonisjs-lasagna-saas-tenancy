...
/**
 * The `@encrypted` / `@searchable` decorator surface end-to-end on real Postgres: a
 * Lucid model transparently encrypts on write and decrypts on read through the async
 * model hooks, and the blind index enables an equality query. This is the ergonomic
 * surface's proof against a real DB and a real CryptoService, plus the fail-closed
 * read after a shred. The decorators are applied functionally (the tsx spec runner
 * rejects a decorated `declare` field; `tsc` validates the `@` sugar), and the runtime
 * path is identical. Self-skips without PG.
 */
...
test.group('crypto @encrypted/@searchable decorators (real pg)', (group) => {
  ...
  test('save encrypts + indexes; the row round-trips as plaintext; the DB holds ciphertext', async ({
    ...
    // On disk it is enc_v2 ciphertext and a non-empty blind index, never plaintext.
  ...
  test('after a crypto-shred, loading the row fails closed (never surfaces ciphertext)', async ({
    ...
    // The row still exists, but decrypting its field on load throws: the decorator
    // never returns the inert ciphertext as if it were plaintext.
