...
/**
 * The DB-level ciphertext CHECK helper (crypto §4 I3, §5 invariant-3): the migration
 * SQL a host adds so a non-enc_v2/enc_v1 write to an encrypted field column is
 * refused by Postgres, closing T5 against the raw-SQL / query-builder / *Quietly
 * bypasses the model hooks cannot see. These prove the emitted SQL and the identifier
