...
// crypto §6.4 Option B: the EncryptedRepository is a context-aware facade over
// CryptoService. It resolves the current tenant from the active scope (so the
// caller passes only subject/category/value), delegates encrypt/decrypt/blindIndex/
// shred, and is fail-closed when there is no tenant scope (never a cross-tenant DEK).
