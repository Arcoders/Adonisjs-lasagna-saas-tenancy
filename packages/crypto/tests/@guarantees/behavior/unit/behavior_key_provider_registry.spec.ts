...
// crypto §7 / ARCHITECTURE §2: the KeyProviderRegistry mirrors the AI/billing extension
// gate — every provider declares a `contractVersion` and register() validates it via
// `assertContractCompat` at registration time (a NEWER contract than this build throws;
// an OLDER or ABSENT one registers with a one-time "unversioned" warning). resolve() is
// fail-closed on an unknown name; the platform never falls back to a weaker/shared key.
test.group('crypto KeyProviderRegistry — contract-version gate + fail-closed resolve', () => {
