 ...
 * holds → the walker's `failed` outcome).
 */
...
test.group('behavior — KEK rotation (rekek)', () => {
  test('classifyRekek: a row at the current generation is current, else rewrap', ({ assert }) => {
    ...
    // No cursor known ⇒ every row is a rewrap candidate (resolved post-hoc).
  ...
  test('current / rotate / failed classification in one pass (cursor path)', async ({ assert }) => {
    ...
    const rotateId = await seed(store, t, 's2', 'identity', 'gen1') // old → rotate
    await seed(store, t, 's3', 'identity', 'genX') // unknown → failed
  ...
  test('post-hoc classification when the provider reports no cursor', async ({ assert }) => {
    ...
    await seed(store, t, 's2', 'identity', 'gen1') // old → rotate
    // reportCursor = false ⇒ classifyRekek returns 'rewrap' for both; the walker
