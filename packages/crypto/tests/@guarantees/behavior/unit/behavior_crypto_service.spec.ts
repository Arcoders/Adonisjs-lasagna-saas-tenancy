...
// The vertical-slice end-to-end proof (crypto §6): a field value is sealed under
// a per-(subject × category) DEK (env KeyProvider wraps the DEK, the enc_v2 seam
// seals the value), and decrypts back only through that DEK. Every failure path
// is fail-closed (I3): a missing DEK or a wrong DEK/category/tenant throws, never
// returns plaintext.
test.group('crypto service — field round-trip under a per-(subject × category) DEK', (group) => {
  ...
  test('a value sealed for one category cannot be read under another (I4/T7)', async ({
    assert,
  }) => {
    const { service } = makeService()
  ...
  test('the second concurrent live provision for one (subject, category) is refused (I10/T12)', async ({
    assert,
  }) => {
