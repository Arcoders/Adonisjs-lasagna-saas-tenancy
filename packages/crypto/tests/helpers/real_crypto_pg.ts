...
export interface RealServiceOpts {
  /** tenantId maps to a physical placement; the driver fake routes `tableLocation` by `tenant.id`. */
...
/** A schema-pg driver fake that routes `tableLocation` by `tenant.id` to its placement. */
 ...
 * and the store's rls branch runs without error; the cross-tenant blocking it provides
...
/** A database-pg driver fake: routes `tenant.id` to its own database + connection. */
