...
test.group('architectural — I2 wrapped-DEK column allowlist', () => {
  ...
  // let a plaintext-DEK column slip the allowlist (the I2 hole the guard exists to close).
  for (const extra of [
  ...
  test('the runner actually reads both wrapped-DEK migrations on disk (no dead guard)', ({
    ...
    // or a rename/move silently drops I2 review (the repo's dead-guard failure mode).
