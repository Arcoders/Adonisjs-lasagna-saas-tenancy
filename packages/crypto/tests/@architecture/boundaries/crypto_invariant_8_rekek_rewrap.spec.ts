...
// The I8 guard is a repo-root script (it runs in `npm run check`); import its pure
...
test.group('architectural — I8 KEK rotation re-wraps DEKs', () => {
  ...
  test('a walker that names openV2WithKey is a violation (field-value decrypt, I8)', ({
    assert,
  }) => {
    const source = [
  ...
  test('a missing walker file is a violation (I8 requires the re-wrap walker)', ({ assert }) => {
