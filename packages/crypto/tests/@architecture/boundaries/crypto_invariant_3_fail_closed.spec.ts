...
// The I3 guard is a repo-root script (it runs in `npm run check`); import its pure
// auditor to exercise fail-closed reads (strict openV2WithKey, no lenient catch) and
// the DB-level ciphertext CHECK backstop that closes T5 for encrypted field columns.
...
test.group('architectural — I3 fail-closed reads + DB CHECK backstop', () => {
  ...
  test('a missing CHECK helper is a violation (T5 backstop absent)', ({ assert }) => {
