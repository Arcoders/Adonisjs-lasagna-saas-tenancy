...
// keyed createHmac (import + call), never a bare unkeyed digest (createHash — incl.
// an aliased import — crypto.hash, or subtle.digest), and no salt column exists on
// the table.
import {
  auditBlindIndex,
...
test.group('architectural — I5 blind index is a keyed HMAC', () => {
  ...
  test('a bare createHash blind index is a violation (brute-forceable, T3)', ({ assert }) => {
    const source = [
