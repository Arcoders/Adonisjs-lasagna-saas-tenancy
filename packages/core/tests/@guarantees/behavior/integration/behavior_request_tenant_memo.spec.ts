...
test.group('request.tenant() — memoization (integration)', () => {
  test('returns the same object reference on repeated calls within one request', async ({
    ...
    try {
      const response = await client.get('/tenant/double-fetch').header('x-tenant-id', tenant.id)
      ...
      assert.isTrue(
        body.sameObject,
        'request.tenant() must return the same object reference on repeated calls'
      )
    } finally {
