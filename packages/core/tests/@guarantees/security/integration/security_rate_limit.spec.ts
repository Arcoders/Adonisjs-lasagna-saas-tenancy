...
test.group('RateLimitMiddleware (integration, real Redis pipeline)', (group) => {
  ...
  test('fail-closed (default): Redis outage → 503 RATE_LIMIT_UNAVAILABLE', async ({ assert }) => {
    ...
    const { default: RateLimitMiddleware } =
      await import('@adonisjs-lasagna/saas-tenancy/middleware').then((m) => ({
        default: m.RateLimitMiddleware,
      }))
    const { default: RateLimitUnavailableException } =
      await import('@adonisjs-lasagna/saas-tenancy/exceptions').then((m) => ({
        default: m.RateLimitUnavailableException,
      }))
