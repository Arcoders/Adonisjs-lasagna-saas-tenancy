...
test.group('DatabasePgDriver (integration)', (group) => {
  ...
  test('reset drops and re-creates the database (sentinel data is wiped)', async ({ assert }) => {
    ...
    const r = await after.rawQuery(`SELECT to_regclass('public.sentinel') IS NOT NULL AS has_table`)
