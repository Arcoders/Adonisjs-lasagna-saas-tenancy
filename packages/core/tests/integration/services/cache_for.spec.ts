...
test.group('cacheFor(tenant) — tenant-scoped cache namespace', (group) => {
  ...
  test('accepts a tenant model (object with .id) as well as a raw id string', async ({
    assert,
  }) => {
    const id = randomUUID()
  ...
  test('cacheFor and getCache().namespace() target the same physical store', async ({ assert }) => {
