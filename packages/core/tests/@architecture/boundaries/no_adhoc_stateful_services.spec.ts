...
// `new` them. The per-subsystem installer modules under
// `providers/installers/*_wiring.ts` (register() slices the provider iterates) are
// the single authority for this list. Keep them in sync when a new stateful
...
// Files allowed to `new` a stateful singleton: the provider's per-subsystem
// installer modules (`providers/installers/*_wiring.ts`, the register() slices —
// matched by path segment so an unrelated future `*_wiring.ts` elsewhere is not
// silently exempted), the billing provider, and each service's own definition
// module. The two `registry.ts` files are likewise matched by path segment (not
// the bare basename) so an unrelated future `registry.ts` is not silently exempted.
// `resolver_chain` is the pure config→registry BUILDER (`buildResolverRegistry`):
// the registry it makes is fully seeded from the immutable config (never
// empty-state), and it is the sync resolution fallback the provider itself wires,
// so it is exempt on the same footing as the installer factories. Paths are
// normalized to forward slashes before matching so this is cross-platform.
const NEW_ALLOWED =
  /(providers\/installers\/[a-z_]+_wiring|billing_provider|tenant_queue_service|circuit_breaker_service|billing_driver_registry|tenant_resolution_cache|tenant_log_context|hook_registry|read_replica_service|bootstrapper_registry|doctor_service|compliance_report_service|audit_log_destination_registry|evaluation_strategy_registry|webhook_transformer_registry|authorizer_registry|tenant_middleware_registry|capability_registry|tenant_scheduler_service|isolation\/registry|resolvers\/registry|resolver_chain)\.ts$/
