...
test.group('TMeta generic — buildTestTenant', (group) => {
  ...
  test('without generic, metadata is the default record type and accepts any keys', ({
    assert,
  }) => {
...
test.group('TMeta generic — MockTenantRepository', (group) => {
  ...
  test('all() returns tenants typed with the generic', async ({ assert }) => {
    const repo = mockTenantRepository<AcmeMetadata>()
    repo.add(
      buildTestTenant<AcmeMetadata>({ metadata: { plan: 'free', trialEndsAt: null, features: [] } })
    )
    repo.add(
      buildTestTenant<AcmeMetadata>({ metadata: { plan: 'pro', trialEndsAt: null, features: [] } })
    )
