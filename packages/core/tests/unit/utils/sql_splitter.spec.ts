...
test.group('splitSqlStatements — adversarial inputs', () => {
  ...
  test('tagged dollar-quoted strings ($BODY$ ... $BODY$) preserve internal semicolons', ({
    assert,
  }) => {
  ...
  test('schema name appearing inside a string literal is not treated as identifier', ({
    assert,
  }) => {
...
test.group('splitSqlStatementsTagged — adversarial inputs', () => {
  ...
  test('rows containing tabs and NULL markers are preserved verbatim', ({ assert }) => {
    const sql = [
      'COPY public.foo (a, b, c) FROM stdin;',
      '1\t\\N\thello world',
      '2\tx\t\\N',
      '\\.',
    ].join('\n')
