...
test.group('validateExternalHttpsUrl — SSRF guard', () => {
  test('accepts a typical external https URL', ({ assert }) => {
    ...
    assert.isNull(
      validateExternalHttpsUrl('https://idp.example.com/.well-known/openid-configuration')
    )
  ...
  test('rejects GCP metadata hostnames by name (defeats DNS-rebinding)', ({ assert }) => {
    assert.equal(
      validateExternalHttpsUrl('https://metadata.google.internal/'),
      'url_blocks_metadata'
    )
  ...
  test('accepts URLs with paths, queries, and ports', ({ assert }) => {
    assert.isNull(validateExternalHttpsUrl('https://idp.example.com:8443/oauth/authorize?foo=1#x'))
  })
})
