...
export default class TenantDoctor extends BaseCommand {
  ...
  @flags.boolean({
    ...
    default: false,
    description:
      'Apply auto-fix for fixable issues (circuit reset, mark stalled tenants as failed)',
  })
  declare fix: boolean
  ...
  async #runWatch(doctor: DoctorService) {
    ...
    const intervalMs = Math.max(MIN_WATCH_INTERVAL_MS, this.interval ?? DEFAULT_WATCH_INTERVAL_MS)
  ...
  #renderWatch(result: DoctorRunResult, intervalMs: number, runMs: number) {
    ...
    for (const report of result.reports) {
      ...
      if (report.issues.length === 0) {
        rows.push([this.colors.green('OK'), report.check, '0', `${report.durationMs}ms`, ''])
        continue
      }
    ...
    for (const r of rows) {
      ...
      this.logger.log(
        `  ${c0}  ${this.colors.bold(c1)}  ${c2}  ${this.colors.dim(c3)}  ${this.colors.dim(r[4])}`
      )
    }
  ...
  #renderReports(result: { reports: any[]; totals: any }) {
    ...
    for (const report of reports) {
      this.logger.log('')
      this.logger.log(
        this.colors.bold(`▸ ${report.check}`) + this.colors.dim(`  (${report.durationMs}ms)`)
      )
