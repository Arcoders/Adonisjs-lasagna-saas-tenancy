...
export default class TenantExec extends BaseCommand {
  ...
  static readonly description = 'Run any ace command inside one or more tenant contexts'
  ...
  @flags.array({
    flagName: 'status',
    required: false,
    description: 'Filter by status (active|provisioning|suspended|failed|deleted). Repeatable',
  })
