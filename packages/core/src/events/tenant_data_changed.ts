...
export interface TenantDataChangePayload {
   ...
   *  mangles class names, so filter on {@link table} instead when you bundle. */
  ...
  /** The row's primary key (column to value). */
...
/**
 * AdonisJS event emitted AFTER a `TracksDataChanges` model's write COMMITS.
