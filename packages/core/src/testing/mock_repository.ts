...
> implements TenantRepositoryContract<TMeta> {
  ...
  async findByIdOrFail(id: string, includeDeleted = false): Promise<TenantModelContract<TMeta>> {
  ...
  async whereIn(ids: string[], includeDeleted = false): Promise<TenantModelContract<TMeta>[]> {
