 ...
 * or the shared database handle — through the sanctioned core-access funnels
 * (`resolveTenantRepository`, `resolveDatabase`). A 403: the plugin is authenticated
 * (installed) but not authorized for core access.
 *
 * READ HONESTLY (S5): this is IN-PROCESS FRICTION, not a hard boundary. It raises
 ...
 * containment is the read-only role (S3) that denies a write regardless of which JS
