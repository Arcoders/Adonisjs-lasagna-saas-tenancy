 ...
 * published version (plan: security-surface, host-pluggable → its own constant).
 */
...
/**
 * A registered authorizer. Discriminated by `kind` (E1) so the `definePlugin`
 * boot dispatcher can `switch` over sections exhaustively; all fields are
 * `readonly`. `name` is branded — it was minted via `authorizerName()`, which ran
...
export default class AuthorizerRegistry {
   ...
   * Reaching the end with no deny allows. Empty chain + no config callback → allow.
   *
   * (The `~1s` deadline-to-deny `executeExtension` wrap and the isthmus/metric
   * emission are layered on in the E3/E5 hardening pass; the fail-closed deny
  ...
  async authorizeAll(
    ...
    for (const entry of this.#ordered()) {
      ...
      try {
        ...
        // than blocking the request. No rateLimit option → no Redis dependency;
