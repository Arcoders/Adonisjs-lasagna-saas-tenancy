 ...
 * (E2 — "nothing wired"). A metric name is a stable operator-facing contract
 ...
 * per-tenant Redis→backoffice sink — because they attribute a RUNTIME fault to the
 * tenant it happened under. Global/config faults (collisions, contract mismatch)
 * go to Isthmus counters instead, never a synthetic tenant id (E3 metric routing).
 */
export const PLUGIN_METRIC = {
  /** A `definePlugin({ onDataChange })` subscriber threw handling a change (SEAM-5). */
