...
/**
 * Best-effort, severity-aware instrumentation for the AI satellite's guards,
 * mirroring the kernel's `emitIsthmusEvent` contract line for line:
 *
 *   1. Counters first, always: `emitAiGuardEvent` synchronously bumps the
 ...
 *      only suppress the EVENT; each suppression increments the dropped
 *      counter, so nothing is ever silently lost.
 ...
 *      guard's throw.
 *
 * Divergence from the kernel, on purpose: the rate-limit WINDOWS are
 * satellite-local. Sharing the kernel's process-wide windows would let an
 * AI-surface burst consume the kernel's per-severity dispatch budget (masking
 * kernel seal/critical events) and would skew the kernel's dropped counters
 * with consumption it cannot see. The budget VALUES are imported from the
 * kernel so both layers stay tuned together; `/internal` is unstable by
 * contract, so if the kernel retunes `ISTHMUS_BUDGETS`, the satellite follows
 * silently (intended).
 *
 ...
 * without faking time.
 */
export function allowAiGuardEvent(severity: IsthmusSeverity, now: number): boolean {
...
export function __resetAiGuardRateLimit(): void {
  windows.clear()
}

// Monotonic in-process counters, bumped BEFORE the rate limiter so totals stay
// exact even when event dispatch drops. Composite string keys keep the maps
// flat; the snapshot re-expands them.
const guardedCounts = new Map<string, number>()
const rejectedCounts = new Map<AiGuardId, number>()
...
export interface AiGuardCountersSnapshot {
...
export function snapshotAiGuardCounters(): AiGuardCountersSnapshot {
  return {
    ...
    rejected: [...rejectedCounts].map(([id, value]) => {
      return { id, severity: aiGuardEntry(id).severity, value }
    }),
...
export function __resetAiGuardCounters(): void {
  guardedCounts.clear()
...
export type AiGuardMetricSink = (
  tenantId: string,
  name: string,
  value: number
) => void | Promise<void>

let metricSink: AiGuardMetricSink | undefined
 ...
 * path.
 */
export function setAiGuardMetricSink(sink: AiGuardMetricSink | undefined): void {
  metricSink = sink
}

export interface AiGuardEmitOptions {
  readonly tenantId?: string | null
  readonly metadata?: Readonly<Record<string, string | number | boolean | null>>
}

type AiGuardDispatcher = (payload: IsthmusGuardTrippedPayload) => Promise<void>

/**
 * The default dispatcher resolves the kernel's public event class lazily (the
 * kernel's own recordScopeBypass pattern), so importing this module never drags
 * app machinery and the module stays safe for bare unit runners.
 * `BaseEvent.dispatch()` throws when the static emitter is unwired (no booted
 * app); the caller's catch absorbs that into dropped{no_emitter}.
 */
const defaultDispatcher: AiGuardDispatcher = async (payload) => {
  const { IsthmusGuardTripped } = await import('@adonisjs-lasagna/saas-tenancy/events')
  await IsthmusGuardTripped.dispatch(payload)
}

let dispatcher: AiGuardDispatcher = defaultDispatcher

/** Test seam: replace the dispatcher without booting an app. Pass undefined to restore. */
export function __setAiGuardDispatcherForTests(fn: AiGuardDispatcher | undefined): void {
  ...
  try {
    ...
    if (sink && options.tenantId) {
      ...
      void Promise.resolve()
        .then(() => sink(tenantId, AI_GUARD_REJECTIONS_METRIC, 1))
        .catch(() => {})
    }

    if (!allowAiGuardEvent(entry.severity, Date.now())) {
    ...
    // Fire-and-forget, with the kernel's exact discipline: capture the
    // dispatcher eagerly (a later test swap can never redirect an in-flight
    // dispatch) and route even a SYNCHRONOUSLY-throwing dispatcher through
    // Promise.resolve().then so the drop is always counted.
