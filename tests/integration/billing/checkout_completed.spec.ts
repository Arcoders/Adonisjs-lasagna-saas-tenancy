...
test.group('checkout.session.completed (integration)', (group) => {
  ...
  test('subscription.created resolves the tenant via the mapping just created', async ({
    ...
    await postSignedEvent(
      ...
      buildEvent(
        ...
        buildSubscription({ id: subId, customer: customerId, productId: 'prod_pro', status: 'active' }),
    ...
    assert.equal(subLedger?.status, 'completed', 'subscription event processed without a resolution error')
  ...
  test('non-subscription/payment checkout modes are a no-op (no mapping written)', async ({
    ...
    const event = buildEvent(
      'checkout.session.completed',
      {
        ...buildCheckoutSession({ tenantId: tenant.id, customerId: `cus_${randomUUID().slice(0, 8)}` }),
        mode: 'setup',
      },
    ...
    assert.isNull(await StripeCustomer.find(tenant.id), 'no mapping for a non-subscription/payment session')
