...
test.group('tenant:billing:doctor + tenant:billing:test-webhook (integration)', (group) => {
  ...
  group.each.setup(async () => {
    ...
    ;(ProcessStripeEventJob as unknown as { dispatch: () => Promise<void> }).dispatch = async () => {}
  })
  ...
  test('tenant:billing:test-webhook POSTs a signed event and the ledger records it', async ({
    ...
    assert.isAtLeast(rows.length, 1, 'the synthetic event was written to the processed-events ledger')
