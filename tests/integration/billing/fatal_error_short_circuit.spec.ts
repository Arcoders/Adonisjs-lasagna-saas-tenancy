...
import { BillingEventDeadLettered } from '@adonisjs-lasagna/saas-tenancy/events'
...
test.group('Fatal-error short-circuit (integration)', (group) => {
  ...
  test('a retryable Stripe error is NOT short-circuited — row stays pending, job re-throws', async ({
    ...
    try {
      ...
      const ledger = await StripeProcessedEvent.find(eventId)
      assert.equal(ledger?.status, 'pending', 'still pending — not marked failed on a transient error')
