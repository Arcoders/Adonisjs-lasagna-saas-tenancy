...
test.group('Doctor: long_running_queries — real PG state', (group) => {
  ...
  test('flags an active backend running pg_sleep above the warn threshold', async ({
    assert,
  }) => {
    ...
    const sample = slow[0]
    assert.equal(sample.severity, 'warn', '3-second sleep must be warn, not critical (60s threshold)')
  ...
  test('returns no issues when no backend exceeds the threshold', async ({ assert }) => {
    ...
    assert.lengthOf(
      slow,
      0,
      'no real query is running over 1h; check must report nothing'
    )
  })
})
