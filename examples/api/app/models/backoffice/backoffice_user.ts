...
/**
 * The operator realm's identity. Lives in `backoffice.backoffice_users`, one
 * fleet-wide table, because BackofficeBaseModel pins every query to the
 * backoffice connection. Access tokens follow the model's adapter, so they
 * land in `backoffice.auth_access_tokens`, never inside a tenant schema.
 *
