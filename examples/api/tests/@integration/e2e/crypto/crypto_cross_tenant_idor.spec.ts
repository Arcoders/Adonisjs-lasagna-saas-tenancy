...
test.group('crypto — hostile cross-tenant IDOR (real HTTP)', (group) => {
  ...
  test('tenant B cannot read or shred tenant A’s secret; A’s DEK never lands in B', async ({
    ...
    // B asks for A's subject → not found: B's schema has no such row (structural).
    ...
    // B tries to shred A's subject → it can only ever reach B's own (empty) schema,
    // so A's DEK is untouched. (A refused/absent-DEK shred is fine; the point is that
    // it CANNOT affect A.)
    ...
    // The wrapped DEK lives only in A's schema. B holds zero — A's key never leaked.
  ...
  test('the same subject id in both tenants derives independent secrets (no cross-open)', async ({
    ...
    // Identical subject + plaintext in BOTH tenants — the DEKs are per-tenant, so the
