...
/**
 * CRYPTO E2E — field encryption end to end through the booted server + real Postgres.
 *
 * This is the crown-jewel proof the package previously lacked (crypto had ZERO E2E):
 ...
 *     column holds enc_v2 ciphertext + a keyed-HMAC blind index, never plaintext;
 ...
 *     T5 bypass the model hooks cannot see).
 */
test.group('crypto — field encryption end to end (real HTTP + real PG)', (group) => {
  ...
  test('encrypt over HTTP → plaintext round-trips; the DB column holds enc_v2 ciphertext', async ({
    ...
    // On disk it is enc_v2 ciphertext + a non-empty blind index, never the plaintext.
  ...
  test('the encrypted-column CHECK rejects a raw plaintext write (T5 backstop, bypassing the model)', async ({
    client,
    assert,
  }) => {
    const t = await createInstalledTenant(client, { plan: 'pro' })
    ...
    // A raw INSERT bypasses the @encrypted hooks — exactly the T5 bypass. The DB
    // CHECK must reject a plaintext value in the guarded column.
    await assert.rejects(() =>
      conn.rawQuery(
