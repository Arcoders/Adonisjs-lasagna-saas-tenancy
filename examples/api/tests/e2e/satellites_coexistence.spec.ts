 ...
 */
test.group('e2e — satellites coexistence (flags + branding + webhooks + audit + metrics + billing)', (group) => {
  group.setup(async () => {
  ...
  group.each.teardown(async () => {
    await db.connection('backoffice').rawQuery('DELETE FROM backoffice.stripe_customers').catch(() => {})
    await dropAllTenants()
  })
  ...
  test('every satellite operates for one tenant in a single lifecycle', async ({
    ...
    // ── billing: inject MockStripe, then checkout ──
    const billing = await app.container.make(BillingService)
    billing.__setStripeForTests(new MockStripe('whsec_demo_placeholder_secret'))
  ...
  test('billing state is isolated per tenant', async ({ client, assert }) => {
    ...
    assert.isNotNull(await StripeCustomer.find(a.id))
    assert.isNull(await StripeCustomer.find(b.id))
