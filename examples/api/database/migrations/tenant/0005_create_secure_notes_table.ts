 ...
 * the per-tenant connection (`tenant_<uuid>`, searchPath set to that schema), so the
 * bare name lands in `tenant_<uuid>.secure_notes`.
 *
 ...
 * The DB-level CHECK from `encryptedColumnCheckSql` is the fail-closed backstop: it
 * rejects a raw, query-builder, or `*Quietly` plaintext write to `secret`, the bypass
 * the model hooks cannot see, so a leak cannot reach disk even outside the model
 * instance path.
 */
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      // The keyed-HMAC blind index for equality search; survives a shred.
