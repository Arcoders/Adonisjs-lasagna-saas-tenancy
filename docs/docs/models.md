...
[Concepts](/docs/concepts) introduced the four layers. This page is the hands-on
...
transaction with `withTenantRls()`. Full detail in [rowscope-pg](/docs/data-isolation/rowscope-pg).
:::
...
package never imports it directly. See [Installation](/docs/installation).
...
[lifecycle events and hooks](/docs/events) rather than per-model hooks.

## Read next

- [Data isolation](/docs/data-isolation/) for the driver each base class pairs with.
- [Tenant identification](/docs/tenant-identification) for how the active tenant is resolved.
- [rowscope-pg](/docs/data-isolation/rowscope-pg) for the shared-schema model in depth.
