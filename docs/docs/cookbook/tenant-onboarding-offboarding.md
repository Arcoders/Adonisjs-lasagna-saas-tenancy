...
`tenant:create` queues an [`InstallTenant` job](/docs/jobs#how-provisioning-flows-through-the-queue)
...
other fields. See [Hooks](/docs/hooks) for the full table.
...
[`QuotaService.assignPlan`](/docs/satellites/quotas) from the same hook, or set
...
2. Optionally `tenant:backup` (the [backup satellite](/docs/satellites/backup))
...
[auto-suspend on payment failure](/docs/satellites/billing#auto-suspend-on-payment-failure).

## Related

- [Hooks](/docs/hooks) — the registry and both phases.
- [Background jobs](/docs/jobs) — where provisioning fires the hooks.
- [Compliance](/compliance) — `tenant:audit:export` and GDPR anonymization.
