...
|---|---|
| **Auth on the admin REST API** | The 36 admin endpoints are mounted with whatever middleware you wire them through. Lasagna ships zero default auth — bring `auth.use('admin').authenticate` or your own. The `multitenancyAdminRoutes()` helper accepts a `resolveAdminActor` callback that the impersonation endpoints require *before* they will issue tokens, so you cannot accidentally forge audit trails by leaving the resolver unset. |
