...
[Satellites](/guides/satellites/) for the full catalog.
...
backstop; [Data isolation](/guides/data-isolation/) covers the trade-off.
...
- [Models](/guides/models); the hands-on reference for the three base classes and
  the row-scope mixin, and which schema each one hits.
- [Tenant identification](/guides/tenant-identification); how
  `resolveTenantId()` picks a UUID from the request.
- [Data isolation](/guides/data-isolation/); the three drivers in
  detail.
- [Bootstrappers](/guides/bootstrappers/); service-level scoping.
