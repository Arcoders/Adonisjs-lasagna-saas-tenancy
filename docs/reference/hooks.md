...
[provisioning sequence](/guides/jobs#how-provisioning-flows-through-the-queue)
...
- [Lifecycle events](/reference/events); fire-and-forget notifications you
  subscribe to, complementary to hooks (which run inline and can abort).
- [Background jobs](/guides/jobs); where provision/destroy hooks actually run.
- [Configuration](/reference/configuration); the full `defineConfig` reference.
