...
- [ ] **Admin API auth.** Pass `middleware` to `multitenancyAdminRoutes(...)` so
      ...
      [Admin REST API](/guides/satellites/admin-rest-api).
...
- [ ] **Transport.** Terminate TLS at the load balancer or Ingress, serve HSTS,
      and add the security-headers middleware from [Deployment](/guides/deployment).
...
[Data isolation](/guides/data-isolation/), [Scaling limits](/guides/scaling-limits),
...
| --------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Postgres down for a resolved tenant     | Typed 503 (`DependencyUnavailableException`), never served central; auto-recovers | Retry; alert on the 5xx rate                                    | [Troubleshooting](/reference/gotchas), [Resilience](/guides/resilience)       |
| Redis down                              | Rate limit fails closed (503 by default); a `fail-open` quota skips enforcement   | Pick the policy per dependency; alert on `DependencyDegraded`   | [Configuration](/reference/configuration), [Troubleshooting](/reference/gotchas) |
| Connection cap saturated                | Exceeds the cap (default) or refuses with 503 (`enforceConnectionCap: true`)      | Raise `maxTenantConnections`, add PgBouncer                     | [Scaling limits](/guides/scaling-limits)                                 |
| Read replica lagging or down            | Still selected; no lag check, no failover                                         | `sticky` + retry-on-primary                                     | [Read replicas](/guides/read-replicas)                                   |
| Provisioning to active race             | Transient 503 (`TenantNotReadyException`) until the schema is ready               | Wait for `TenantActivated` before redirecting in                | [Troubleshooting](/reference/gotchas)                                       |
| SSRF DNS rebinding (TOCTOU)             | URL validated, resolved IP not pinned                                             | Network egress controls                                         | [Troubleshooting](/reference/gotchas), [Security](/guides/security)                |
| Top-level `orWhere` under `rowscope-pg` | Can widen a query past the tenant predicate                                       | Nested `where` group + the RLS backstop                         | [rowscope-pg](/guides/data-isolation/rowscope-pg)                        |
| Disk or Postgres resource exhaustion    | Provisioning and writes fail; the cap does not bound disk / CPU / IOPS            | Monitor those separately; re-run provisioning after remediation | [Troubleshooting](/reference/gotchas)                                       |
| Multi-pod cache drift                   | Per-process cache serves stale tenant data                                        | Shared Redis cache store                                        | [Deployment](/guides/deployment)                                         |
...
1. **Confirm the driver and its contract.** For `rowscope-pg`, verify the RLS
   ...
   app-side cause; see [Troubleshooting](/reference/gotchas).
...
- **Replica down or lagging:** there is no failover, so route reads to the primary
  ...
  [Read replicas](/guides/read-replicas).

## Read next

- [Deployment](/guides/deployment); the Docker and Kubernetes artifacts.
- [Security](/guides/security); the hardening guarantees behind this checklist.
- [Resilience](/guides/resilience); how the package degrades under dependency outages.
