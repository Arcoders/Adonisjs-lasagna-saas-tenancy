...
| `TenantAccessForbiddenException` | `403` | `E_TENANT_ACCESS_FORBIDDEN` | The host's optional [`authorizeTenantAccess`](/guides/security#what-the-host-owns) hook returned `false` for the resolved tenant. Thrown by the guard middleware; it is the seam that lets a host reject a caller who is not a member of the tenant. |
...
| `TenantConnectionLimitException` | `503` | `E_TENANT_CONNECTION_LIMIT` | Either `isolation.enforceConnectionCap` is on and the budget is exhausted with every open connection inside the eviction grace window, or the pool reached the absolute `isolation.maxTenantConnectionsHardCeiling` (which refuses regardless of `enforceConnectionCap`). |
...
- [Services API](/reference/services); where these exceptions originate.
- [Resilience](/guides/resilience); the policy behind `DependencyUnavailableException`.
- [Troubleshooting](/reference/gotchas); symptoms and fixes when these fire.
