...
| `isolation.maxTenantConnectionsHardCeiling` | `number?` | unset | Absolute upper bound above `maxTenantConnections`: once the pool reaches it, `connect()` refuses a new tenant connection with a 503 **regardless of `enforceConnectionCap`**, so an availability-favouring pool can never exhaust PG's `max_connections`. **Unbypassable**: unlike the soft cap, operational paths (provisioning, migrations) do *not* bypass it, and bulk/parallel operational paths draw from `operationalConnectionBudget`. Set it generously above the soft cap. |
| `isolation.operationalConnectionBudget` | `number` | `4` | How many connections the bulk/parallel operational paths (migration worker pool, warm pool, satellite migration temp clones) may hold open at once. Kept small so operational work never crowds out request-serving connections; additionally clamped at runtime to `maxTenantConnectionsHardCeiling`, so a parallel/warm operation can never push the pool past the absolute ceiling. |
| `isolation.migrateOnProvision` | `boolean` | `false` | Migrate **and seed** a tenant to head as part of provisioning, before it is marked `active`. Off by default (provision and migrate stay separate lifecycles). With it `true`, `InstallTenant` provisions, fires the provision hooks, then applies all pending migrations and runs the `after:migrate` seeders (via `healTenant`) and only then flips the tenant to `active` — so a UI-created tenant is born fully migrated and can never land in the active-but-unmigrated state that serves 500s and trips the doctor. A migration failure leaves the tenant `failed`, never a half-baked `active`. Idempotent with a separate fleet-migrate step. |
| `isolation.warmPool.enabled` | `boolean` | `false` | **F2 warm pool.** Pre-open connections at boot for the declared hot tenants so their first request skips the cold-connection cliff. Off unless `true`. |
| `isolation.warmPool.count` | `number` | `1` | Connections to pre-open per declared tenant. The absolute ceiling still caps the total. |
| `isolation.warmPool.tenants` | `string[]` | | **Operator-declared** hot-tenant ids to warm. Never derived from request input (so it can't be an attacker-forced warm DoS). Ignored when `selectTenants` is set. |
| `isolation.warmPool.selectTenants` | `() => string[] \| Promise<string[]>` | | Authenticated policy hook returning the tenant ids to warm (e.g. "top N by traffic"), called once at boot in the app's own context. Alternative to a static `tenants` list. |
| `isolation.pgBouncer.mode` | `'auto' \| 'transaction' \| 'session' \| 'off'` | `off` | **F3 PgBouncer awareness.** `auto` never *assumes* transaction pooling (declare `transaction`/`session` explicitly for a confirmed mode); the tenant routing path is transaction-pooling-safe regardless. Detection is informational and never raises a cap or the ceiling. The `pgbouncer` doctor check reports the posture. |
| `isolation.pgBouncer.autoTuneCaps` | `boolean` | `false` | When a transaction-pooling posture is confirmed, allow a conservative auto-tune. Detection may only confirm or tighten, never raise a cap. |
