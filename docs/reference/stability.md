...
- Want true tenant isolation and nothing else? You are on `release-candidate`
  ground: the core. Pin the version, follow the [deployment](/guides/deployment)
  and [security](/guides/security) guides, and you are leaning only on what is tested
  and gated in CI.
...
- [Upgrade to 1.0](/reference/upgrade-to-1.0); the mechanical migration from 0.x.
