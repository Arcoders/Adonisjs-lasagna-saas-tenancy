...
|---|---|---|
| **CC6.1** logical access | [`authorizeTenantAccess`](/guides/security) membership gate + [tenant isolation](/guides/data-isolation/) | Identity/membership source, full RBAC |
...
| **CC7.2** system monitoring | [Append-only audit log](/guides/satellites/audit) + `tenant:doctor` + [metrics/health](/guides/health) | Alerting, SIEM, on-call |
...
| **C1** confidentiality | [Isolation drivers](/guides/data-isolation/) (`schema-pg` / `database-pg` / `rowscope-pg`) | Data classification |
...
| **Art.30** records of processing | [Append-only audit log](/guides/satellites/audit) | Which events you record via `audit.log()` |
...
privileged role; see [Audit logs → Retention](/guides/satellites/audit#retention).

**Data residency.** Use the [`database-pg`](/guides/data-isolation/database-pg)
...
- [Security](/guides/security); the guarantees these controls build on.
- [Audit logs](/guides/satellites/audit); append-only enforcement and retention.
- [Data isolation](/guides/data-isolation/); choosing schema/database/row scoping.
- [Production checklist](/reference/production-checklist); the hardening runbook.
