...
- **Header-domain disagreement**: by default the verified `Host`-resolved
  ...
  [Routing's strict mode](/guides/routing#strict-mode-the-default).

## Read next

- [Branding satellite](/guides/satellites/branding); where
  `custom_domain` is stored.
- [Routing, custom domain mapping](/guides/routing#custom-domain-mapping)
