...
[Health & metrics](/guides/health). `quotas` prints a `plans` block to paste into
...
[Stripe + quotas](/guides/cookbook/stripe-quotas) and the
[Billing satellite](/guides/satellites/billing) reference.
...
- [Stripe + quotas](/guides/cookbook/stripe-quotas); the billing journey
  end to end.
- [Satellites](/guides/satellites/); the reference for each feature.
- [Installation](/start/installation); the first-time setup this builds on.
