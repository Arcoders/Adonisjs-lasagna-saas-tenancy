...
2. After the retention window elapses, `tenant:purge-expired` calls
   ...
   driver](/guides/data-isolation/database-pg) does terminate sessions,
...
- **You need strict OS-level isolation** (per-tenant `CREATEDB` /
  ...
  [`database-pg`](/guides/data-isolation/database-pg).
...
- **You're writing unit tests** and don't want a real PG → use
  [`sqlite-memory`](/guides/data-isolation/sqlite-memory).

## Operational notes

- `pg_dump --schema=tenant_<uuid>` produces a portable per-tenant
  archive. Lasagna's [`tenant:backup`](/reference/commands#tenant-backup)
...
- [database-pg driver](/guides/data-isolation/database-pg); one database per tenant.
