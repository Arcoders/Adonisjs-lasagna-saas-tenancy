...
| **Unsafe resolution posture fails closed at boot (production)** | One `resolutionSafetyAudit` covers both cross-tenant resolution risks: a client-controlled strategy (header/path/request-data) with no `authorizeTenantAccess` gate (IDOR), and a host strategy with no `resolver.expectedHostSuffix` allowlist. In production an unacknowledged finding throws at boot; in dev the same messages warn and the `membership_gate` doctor check reports them. Acknowledge a consciously-handled IDOR with `acknowledgeNoMembershipGate: true`. |
