...
|---|---|
| **No DDL injection via `tenant.id`** | `assertSafeIdentifier()` rejects anything that could escape a quoted PG identifier (`"`, `;`, whitespace, shell metacharacters, length > 63) at every driver entry that interpolates the id into DDL. It also rejects any non-canonical (non-NFKC) input rather than folding it, so a homoglyph like `tenant_℀` is refused instead of silently collapsed onto `tenant_a` and colliding with a real schema. |
...
| **Scope bypass leaves an audit trail** | `unscoped(fn)` is the single seam that turns off tenant scoping. Every outermost bypass emits a rate-limited, best-effort `scope:bypass` audit event with the caller's `reason` and the active tenant context, so a deliberate or accidental cross-tenant escape is never invisible. Pass `unscoped(fn, { reason })` to attribute it. A static guard pins that the bypass storage has exactly one door and it routes through the audit. |
...
| **Secrets fail closed and domain-separated** | Every stored secret (webhook signing secret, SSO `client_secret`) reads and writes through one accessor (`readSecret` / `writeSecret`). Reads use `decryptStrict`, so a plaintext, corrupted, or tampered value throws instead of being used. Each secret class encrypts under its own HKDF context, so a ciphertext written for one class never decrypts as another. A static guard forbids any other module from calling the raw crypto primitives on a secret. |
| **Outbound fetch pins the validated address (closes DNS rebinding)** | Webhook delivery and SSO token/discovery calls go through one `safeFetch` seam. It resolves the host once, rejects any private/loopback/link-local/metadata address, then connects only to that pinned address while presenting the original hostname for Host and TLS SNI, so a name that rebinds after the check can never be reached. Redirects are never followed. First-party billing API hosts use a trusted-host allowlist (no pin, so CDN rotation keeps working). A static guard forbids raw `fetch`/`node:http(s)` outside the seam. |
...
| **Unsafe resolution posture fails closed at boot (outside dev/test)** | One `resolutionSafetyAudit` covers both cross-tenant resolution risks: a client-controlled strategy (header/path/request-data) with no `authorizeTenantAccess` gate (IDOR), and a host strategy with no `resolver.expectedHostSuffix` allowlist. Outside a recognized dev/test environment — production, `staging`, or any unrecognized/unset `NODE_ENV` — an unacknowledged finding throws at boot; in dev/test the same messages warn and the `membership_gate` doctor check reports them. Acknowledge a consciously-handled IDOR with `acknowledgeNoMembershipGate: true`. |
...
- [ ] Separate auth realms: operators authenticate through their own guard and user model on the backoffice/central layer (never a table inside a tenant schema), with MFA and short token lifetimes; tenant users get their own guard on a `TenantBaseModel`. See [Authentication](/guides/authentication).
