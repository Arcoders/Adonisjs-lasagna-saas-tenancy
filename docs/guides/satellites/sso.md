...
OIDC discovery runs internally whenever the service builds an authorization URL or handles
a callback; it is a private step of `SsoService`, not a method you call. The discovery
document is cached for one hour and hardened before it is trusted:

- Verifies the discovery doc's `issuer` matches the requested issuer
  (OIDC Discovery 1.0 §4.3).
- Applies the SSRF guard to the discovered `token_endpoint` and
  `jwks_uri`; rejects loopback, RFC 1918, link-local, cloud metadata,
  and IPv6-bracketed hosts.
...
Beyond `register(provider)`, the `identityProviderRegistry` exposes a small
introspection surface for tests and tooling:

| Member | Returns | Purpose |
|---|---|---|
| `get(name)` | `IdentityProviderContract \| undefined` | The registered provider for a name. |
| `has(name)` | `boolean` | Whether a provider is registered. |
| `list()` | `readonly string[]` | Every registered provider name. |
| `clear()` | `this` | Drop all registrations (chainable; mainly for test isolation). |
| `contractVersion` (getter) | `number` | The `SSO_CONTRACT_VERSION` this build implements. |

## Read next

- [Authentication](/guides/authentication); how SSO composes with tenant auth.
- [Security](/guides/security); the replay and state guarantees.
