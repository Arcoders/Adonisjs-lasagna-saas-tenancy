...
assignment in the [quotas satellite](/guides/satellites/quotas), backed
...
[Stripe + quotas (cookbook)](/guides/cookbook/stripe-quotas). This page
...
portal, follow [Stripe + quotas](/guides/cookbook/stripe-quotas).
...
`QuotaService.assignPlan` for you, and the [quotas satellite](/guides/satellites/quotas)
...
| `notifyOnQuotaExceeded` | `boolean?` | `false` | Dispatch `QuotaWarningMailer` on `TenantQuotaExceeded`. Requires `@adonisjs/mail`. The per-(tenant, quota) dedupe (24h, Redis `SETNX`) is advisory and fails open: a Redis outage sends without dedupe rather than dropping the warning. |
| `suspendOnPaymentFailure` | `boolean?` | `false` | Auto-suspend a tenant on a terminal payment failure (`PaymentFailed{final:true}` or `SubscriptionCanceled{reason:'dunning_failed'}`); blocks API access and dispatches `TenantSuspended`. |
| `reactivateOnPaymentSuccess` | `boolean?` | `false` | When `suspendOnPaymentFailure` is on, auto-reactivate on `PaymentSucceeded` (dispatches `TenantActivated`). Ignored otherwise. |
...
| `fiscal.enabled` | `boolean?` | `false` | Opt-in fiscal runtime: snapshot the provider's tax breakdown onto events/ledger, write `billing_invoice_snapshots`, and mount the invoice read routes. DDL is published at configure time. See [Fiscal features](#fiscal-features-opt-in). |
| `fiscal.automaticTax` | `boolean?` | `false` | Pass Stripe `automatic_tax: { enabled: true }` at checkout so the provider computes tax; we only snapshot the result. |
...
This mirrors the [isolation driver](/guides/data-isolation/) seam: one
...
via the BentoCache redis bus. The row records where the assignment came
from in its `source` column, typed as the exported `TenantPlanSource`
(`'stripe' | 'manual' | 'backfill'`, plus any custom string such as
`'dunning'`; from `@adonisjs-lasagna/saas-tenancy/models/satellites`).
...
`@adonisjs-lasagna/billing`. Wire it into your
`HealthService`:
...
// Provider-agnostic: drive BillingService through an in-memory driver. Resolve the
// same registry the billing provider registered as a singleton, register a mock
// driver, and activate it — that's the whole of what the active-driver lookup reads.
...
- [Stripe + quotas (cookbook)](/guides/cookbook/stripe-quotas); the
  end-to-end recipe.
- [Quotas satellite](/guides/satellites/quotas); where the plan
  assignments land.
- [Lifecycle events](/reference/events): the 10 billing events plus the
  14 tenant-lifecycle events.
- [Resilience → billing failure modes](/guides/resilience#billing-satellite-failure-modes-and-recovery); the failure/recovery map for the webhook pipeline.
