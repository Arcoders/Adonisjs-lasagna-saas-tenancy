...
The commands above are thin wrappers over four services, each resolvable from the container
(`app.container.make(...)`) and importable from `@adonisjs-lasagna/backup` when you want to
drive a backup operation from your own code.

| Service | Method | Returns |
|---|---|---|
| `BackupService` | `backup(tenant)` | `Promise<BackupMetadata>` — dump (and optionally upload) a tenant's schema + data. |
| | `restore(tenant, fileName)` | `Promise<void>` — restore a named archive (symlink-checked). |
| | `listBackups(tenantId)` | `Promise<BackupMetadata[]>` |
| | `deleteBackup(tenantId, fileName)` | `Promise<void>` |
| `CloneService` | `clone(source, destination, { schemaOnly, clearSessions })` | `Promise<CloneResult>` — provision a tenant from another. |
| `SqlImportService` | `import(tenant, filePath, { sourceSchema, dryRun, strict?, force? })` | `Promise<SqlImportResult>` — import a `pg_dump` `.sql` into a tenant. |
| `BackupRetentionService` | `getTierFor(tenant)` / `shouldBackup(tenant)` | `Promise<BackupRetentionTier>` / `Promise<boolean>` |
| | `planRetention(tenant)` / `applyRetention(tenant)` | `Promise<RetentionPlan>` — compute, then enforce, which archives to keep. |

The `CloneTenant` background job carries a `CloneTenantPayload`:

```ts
import type { CloneTenantPayload } from '@adonisjs-lasagna/backup'

interface CloneTenantPayload {
...
- [Doctor](/reference/commands#doctor); the `backup_recency` and `backup_encryption` checks.
- [Deployment](/guides/deployment); the client-tool and storage requirements.
