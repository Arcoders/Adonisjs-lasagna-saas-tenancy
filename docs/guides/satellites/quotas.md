...
[resilience policy](/guides/resilience), so a Redis outage follows
...
and [Troubleshooting](/reference/gotchas#fail-open-quotas-silently-stop-enforcing).
...
- [Lifecycle events](/reference/events); the `TenantQuotaExceeded` event.
- [Configuration](/reference/configuration); the plan and quota options.
