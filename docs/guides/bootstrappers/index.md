...
|---|---|---|
| [`cacheBootstrapper`](/guides/bootstrappers/cache) | BentoCache | `cacheFor(tenant)` — keys namespaced under `tenant:<id>` |
| [`driveBootstrapper`](/guides/bootstrappers/filesystem) | `@adonisjs/drive` | `tenantDisk(disk?)` — keys prefixed with `tenants/<id>/` |
| [`mailBootstrapper`](/guides/bootstrappers/mail) | `@adonisjs/mail` | `tenantMailer(transport?)` — stamps `X-Tenant-Id` on every message |
| [`sessionBootstrapper`](/guides/bootstrappers/session) | `@adonisjs/session` | `tenantSession(ctx)` / `tenantSessionKey(key)` — keys prefixed with `tenants/<id>/` |
| [`transmitBootstrapper`](/guides/bootstrappers/broadcasting) | `@adonisjs/transmit` | `tenantBroadcast(channel, payload)` / `tenantChannel(name)` — channels prefixed with `tenants/<id>/` |
...
[Database routing](/guides/bootstrappers/database) page for the
mechanism.
:::
...
- [Testing](/guides/testing). How to swap bootstrappers for hermetic
  fakes.
